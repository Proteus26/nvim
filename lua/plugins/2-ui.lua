...
return {
  ...
  {
    "catppuccin/nvim",
    -- Load immediately so the colorscheme is available when init calls it.
    lazy = false,
    name = "catppuccin",
    opts = {
      flavour = "mocha",
  ...
  --  Be aware some components are positional. Read heirline documentation.
  {
    ...
    config = function(_, opts)
      ...
      -- We are using lualine for the statusline now; disable heirline's statusline
      -- to avoid conflicts while preserving tabline/winbar/statuscolumn.
      opts.statusline = nil
      heirline.setup(opts)
    end,
  },
  ...
  --  Theme is provided by Catppuccin and will follow the configured flavour (mocha).
  {
    ...
    lualine_z = {
  {
    ...
    fmt = function(str)
      -- keeps the right bubble visually consistent in width
